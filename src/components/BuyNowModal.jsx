...
export default function BuyNowModal({ product, open, onClose }) {
  ...
  const [address, setAddress] = useState("Shop #10, First Floor, Emporium Plaza, Bank Road, Rawalpindi")
      ...
      {open && (
        ...
        >
          <motion.div
            ...
            className="w-full sm:max-w-lg rounded-t-2xl sm:rounded-2xl max-h-[90vh] overflow-hidden"
          ...
          >
            <div className="overflow-y-auto max-h-[90vh]">
              <div className="flex items-center justify-between px-4 pt-4 pb-0 sm:px-6 sm:pt-6">
                ...
                <button
                  onClick={onClose}
                  className="p-1.5 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-800 transition-all shrink-0"
                >
              ...
              <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-3">
                {product.storageOptions?.length > 0 && (
                  <div>
                    <label className="block text-xs font-semibold mb-1.5" style={{ color: "var(--text-secondary)" }}>STORAGE</label>
                ...
                <div className="flex items-center gap-3 p-2.5 rounded-xl" style={{ background: "var(--bg-main)" }}>
                  {product.images?.[0] && (
                    <img src={product.images[0]} alt="" onError={(e) => { e.target.src = "https://placehold.co/48x48/e2e8f0/64748b?text=P" }} className="w-10 h-10 object-cover rounded-lg" />
                  )}
                  <div>
                    <p className="font-semibold text-xs" style={{ color: "var(--text-main)" }}>{product.name}</p>
                ...
                <button
                  type="submit"
                  className="w-full btn-primary text-center flex items-center justify-center gap-2 py-3 text-sm"
                >
